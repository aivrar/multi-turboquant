...
BUILTIN_ENVIRONMENT_PROFILES = (
    ...
    DependencyProfile(
        ...
        packages=(
            ...
            "transformers==4.57.6",
            "peft==0.20.0",
            "kvpress @ git+https://github.com/NVIDIA/kvpress.git@161705a68f64df329c88a6da5f20300a89aa7542",
        ),
        supported_os=("linux",),
        supported_compute=("cuda",),
        required_executables=("git",),
        ...
        package_indexes=(("pytorch-cu126", "https://download.pytorch.org/whl/cu126"),),
        torch_cuda_major=12,
        validation_modules=("torch", "transformers", "peft", "kvpress"),
        notes=(
            "Pins the reviewed KVPress implementation that includes RestoreKVPress.",
            "Inference still downloads a model-specific RestoreKV adapter from Hugging Face.",
            "Import validation does not establish the quality of a selected cache budget.",
        ),
        local_source_package="kvpress",
        local_source_markers=(
            "pyproject.toml",
            "kvpress",
    ...
    DependencyProfile(
        ...
        packages=(
            "torch==2.7.1",
            "transformers==4.57.6",
            "archead-llm @ git+https://github.com/suayptalha/archead.git@29002720037fa4215822933e3ec50636882a7cc2",
        ),
        supported_os=("linux",),
        supported_compute=("cuda",),
        required_executables=("git",),
        ...
        package_indexes=(("pytorch-cu126", "https://download.pytorch.org/whl/cu126"),),
        torch_cuda_major=12,
        validation_modules=("torch", "transformers", "archead"),
        notes=(
            "Installs the base Python implementation without optional Triton or baseline packages.",
            "Compression requires representative hidden states and access to the dense output head.",
            "The result remains a Transformers model artifact, not a GGUF patch.",
        ),
        local_source_package="archead-llm",
        local_source_markers=("pyproject.toml", "archead", "README.md"),
    ),
    DependencyProfile(
        id="novakv",
        name="NOVA-KV research runtime",
        optimization_id="novakv",
        ...
        supported_compute=("cuda",),
        blocked_reason=(
            "NOVA-KV is a complete research fork of SGLang 0.5.10 with model-specific "
            "calibration artifacts; it is not safe to install as an add-on to another engine."
        ),
        notes=(
            "Use its own checkout and reproduce BF16-versus-NOVA quality and throughput gates.",
        ),
        installable=False,
    ),
    DependencyProfile(
        id="dspark",
        name="DSpark serving research",
        optimization_id="dspark",
        ...
        blocked_reason=(
            "DSpark depends on a matching trained drafter and an exact vLLM serving path; "
            "recent runtime reports show DSpark-specific proposer and hidden-state failures."
        ),
        notes=(
            "Keep it unavailable until one reviewed vLLM release passes output-parity and concurrency gates.",
        ),
        installable=False,
    ),
    DependencyProfile(
